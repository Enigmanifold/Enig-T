...
cam1_num=find(cameras.cameraId==images(viewSel(1)).camera);
cam2_num=find(cameras.cameraId==images(viewSel(2)).camera);
...
% TODO: select different images by changing "viewSel" you may need to manually select images that have enough overlap
R1 = quat2rotmSelf(images(viewSel(1)).quat);
R2 = quat2rotmSelf(images(viewSel(2)).quat);
T1 = transpose(images(viewSel(1)).T);
T2 = transpose(images(viewSel(2)).T);
C1 = -inv(R1) * T1;
C2 = -inv(R2) * T2;
R_gt = R2 * R1';
T_gt = R2 * (C1 - C2);
