function chosen_phis=calc_phi3_multi(epi12_sph,p1,p2,ptsnum,diff_amplifier)
    ...
    phi1_c0std=std((phis_c0(1,:)-ones(1,ptsnum).*mean1).*diff_amplifier);
    phi2_c0std=std((phis_c0(2,:)-ones(1,ptsnum).*mean2).*diff_amplifier);
    phi3_c0std=std((phis_c0(3,:)-ones(1,ptsnum).*mean3).*diff_amplifier);
    phi4_c0std=std((phis_c0(4,:)-ones(1,ptsnum).*mean4).*diff_amplifier);
